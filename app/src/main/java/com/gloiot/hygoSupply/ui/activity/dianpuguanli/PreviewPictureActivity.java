...
//                PictureUtlis.loadImageViewHolder(mContext,url, R.mipmap.weixin, photoView);
                CommonUtils.setDisplayImageOptions(photoView, url, 0);
            }
            ...
            container.addView(photoView, LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT);
            return photoView;
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.tvDownload:
                if (imagePathList != null) {
                    if (!TextUtils.isEmpty(imagePathList.get(currentViewPosition))) {
                        progressDialog.show();
