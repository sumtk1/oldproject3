...
import com.gloiot.hygoSupply.ui.activity.wode.shouyi.ZhuanYHKXiangQingActivity;
...
public class ZhangDanMessageActivity extends BaseActivity {
    ...
    public void initData() {
        ...
        CommonUtils.setTitleBar(this, "账单通知");
        if (type.equals("no")) {
            lvMessage.setVisibility(View.GONE);
            llNodata.setVisibility(View.VISIBLE);
            tvNodata.setText("暂无账单通知");
        } else {
            ...
            BroadcastManager.getInstance(mContext).addAction(MessageManager.NEW_MESSAGE, new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    if (imMsgBean.getSendid().equals("003")) {
                        getData(true);
                    }
                }
            });
            ...
            lvMessage.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                ...
                public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                    ...
                    String title = "";
                    try {
                        iD = new JSONObject(list.get(position).getMessage()).getString("id");
                        type = new JSONObject(list.get(position).getMessage()).getString("type");
                        title = new JSONObject(list.get(position).getMessage()).getString("title");
                    } catch (JSONException e) {
                        e.printStackTrace();
                    }
                    Intent intent = new Intent(mContext, ZhuanYHKXiangQingActivity.class);
            ...
            lvMessage.setOnScrollListener(mScrollFirstItem);
        }
    }
        ...
        public void onScrollStateChanged(AbsListView view, int scrollState) {
            switch (scrollState) {
                ...
                case AbsListView.OnScrollListener.SCROLL_STATE_IDLE:
                    //判断滚动到底部
                    if (lvMessage.getLastVisiblePosition() == (list.size() - 1)) {
    ...
    private void getData(boolean isNewest) {
        ...
        if (isNewest) {
            list.clear();
            onceList = imdbManager.queryChatMsg("003", 0);
        } else {
            onceList = imdbManager.queryChatMsg("003", list.size());
        }
    ...
    private void setAdapter() {
        commonAdapter = new CommonAdapter<ImMsgBean>(mContext, R.layout.item_message_zhangdan, list) {
            @Override
            public void convert(ViewHolder holder, ImMsgBean systemMessageBean) {
                ...
                if (systemMessageBean.getMsgtype().equals("图文_0")) {
                    ...
                    try {
                        ...
                        holder.setText(R.id.tv_message_content, jsonObject.getString("title"));
                        holder.setText(R.id.tv_money, "￥" + jsonObject.getString("integral"));
