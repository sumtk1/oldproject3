...
import com.gloiot.chatsdk.chatui.keyboard.interfaces.EmoticonFilter;
import com.gloiot.chatsdk.chatui.keyboard.utils.EmoticonsKeyboardUtils;
import com.gloiot.chatsdk.chatui.keyboard.widget.EmoticonSpan;
...
public class EmojiFilter extends EmoticonFilter {
    ...
    public void filter(EditText editText, CharSequence text, int start, int lengthBefore, int lengthAfter) {
        ...
        String originalData = text.toString().substring(start, text.toString().length());
        for (int i = 0; i < originalData.length(); i++) {
            int codePoint = originalData.codePointAt(i);
            if (codePoint > 100000) {
//                String key="0x"+Integer.toHexString(codePoint);
                String key = new String(Character.toChars(codePoint));
                if (null != DefQqEmoticons.sQqEmoticonHashMap1.get(key)) {
                    int icon = DefQqEmoticons.sQqEmoticonHashMap1.get(key);
                    if (icon > 0) {
                        emoticonDisplay(editText.getContext(), editText.getText(), icon, emoticonSize, start + i, start + i+2 );
                    }
                }
            }
        }
...
//        }
    ...
    public static Spannable spannableFilter(Context context, Spannable spannable, CharSequence text, int fontSize, EmojiDisplayListener emojiDisplayListener) {
        String originalData = text.toString();
        for (int i = 0; i < originalData.length(); i++) {
            int codePoint = originalData.codePointAt(i);
            if (codePoint > 100000) {
//                String key="0x"+Integer.toHexString(codePoint);
                ...
                if (null != DefQqEmoticons.sQqEmoticonHashMap1.get(key)) {
                    ...
                    if (emojiDisplayListener == null) {
                        if (icon > 0) {
                            emoticonDisplay(context, spannable, icon, fontSize, i, i + 2);
                        }
                    } else {
                        emojiDisplayListener.onEmojiDisplay(context, spannable, "" + icon, fontSize, i, i + 2);
                    }
